...
const SHELL_BYPASS_ROUTES = new Set(['/complete-profile', '/hub']);
...
const pageTitleMap = {
  ...
  '/job-creation': 'Create Printing Job',
...
const AppShell = ({ user, children }) => {
  ...
  return (
    <div className="app-global-layout min-h-screen bg-slate-50 text-slate-900">
      ...
      <div className={`app-global-content ${isSidebarOpen ? 'md:ml-72' : 'md:ml-0'}`}>
        <header className="lg-glass-bar sticky top-0 z-30 flex h-16 items-center justify-between border-b border-slate-200 bg-white px-4 md:px-6">
